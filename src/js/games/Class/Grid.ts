...
export default class Grid extends TurnBasedGame {
  ...
  shuffleCells(): any {
    ...
    let k = 0;
    let tag = arrayRandomIndexTags.findIndex(
      (i) => this.#tagOpacity.value === i
    );
    ...
    const ARRAY_NO_TAG_OPACITY = arrayRandomIndexTags.filter(
      (i) => this.#tagOpacity.value !== i
    );
    ...
    arrayRandomIndexTags.forEach((element, index) => {
      ...
      if (this.#cells[index].tag.value === this.#tagOpacity.value)
        this.#cellOpacity = this.#cells[index];
    });
  }
  ...
  move(cell: Cell) {
    if (this.#cellOpacity.y > cell.y) {
      this.arrayShiftToRight(
        this.cellsByColumn[cell.x].slice(cell.y, this.#cellOpacity.y + 1)
      );
    }
    if (this.#cellOpacity.y < cell.y) {
      this.arrayShiftToRight(
        this.cellsByColumn[cell.x]
          .slice(this.#cellOpacity.y, cell.y + 1)
          .reverse()
      );
    }
    if (this.#cellOpacity.x > cell.x) {
      this.arrayShiftToRight(
        this.cellsByRow[cell.y].slice(cell.x, this.#cellOpacity.x + 1)
      );
    }
    if (this.#cellOpacity.x < cell.x) {
      this.arrayShiftToRight(
        this.cellsByRow[cell.y].slice(this.#cellOpacity.x, cell.x + 1).reverse()
      );
    }
  }
  ...
  moveCheck(cell: Cell): boolean {
    if (
      (this.#cellOpacity.x !== cell.x && this.#cellOpacity.y === cell.y) ||
      (this.#cellOpacity.x === cell.x && this.#cellOpacity.y !== cell.y)
    )
      return true;
    return false;
